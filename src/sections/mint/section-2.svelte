<div
	class="mb-16 grid h-full w-full bg-primary-dark-variant sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
	<div class="h-full w-full">
		<video width="100%" height="auto" autoplay autobuffer loop muted>
			<source src="/assets/vids/nft-0.mp4" type="video/mp4" />
		</video>
	</div>
	<div class="h-full w-full">
		<video width="100%" height="auto" autoplay autobuffer loop muted>
			<source src="/assets/vids/nft-1.mp4" type="video/mp4" />
		</video>
	</div>
	<div class="h-full w-full">
		<video width="100%" height="auto" autoplay autobuffer loop muted>
			<source src="/assets/vids/nft-2.mp4" type="video/mp4" />
		</video>
	</div>
	<div class="h-full w-full">
		<video width="100%" height="auto" autoplay autobuffer loop muted>
			<source src="/assets/vids/nft-3.mp4" type="video/mp4" />
		</video>
	</div>
</div>
